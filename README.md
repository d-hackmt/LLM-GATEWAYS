...
GATEWAY_CONFIG = {
    ...
    "cache": {"mode": "simple"},        # exact-match cache (free plan)
    "retry": {
        "attempts": 2,
...
**Slugs:** `flight-policsy` and `flight-policy` are the Portkey dashboard integration names for the two Groq API keys. The fallback key is stored in `.env` as `GROQ_FALLBACK_API_KEY` — Portkey reads it from the `flight-policy` dashboard integration, not from code.
